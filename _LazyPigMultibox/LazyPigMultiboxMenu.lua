local CheckBoxTables = {
	...
	["Assist Master"] = {
		...
		[5] = { "LazyPigMultiboxCheckbox11", "Improved Targeting" },
	},
	...
	["Follow Master Actions"] = {
		...
		[3] = { "LazyPigMultiboxCheckbox22", "Dismount" },
	...
	["Use Predefined Class' Script"] = {
		...
		[3] = { "LazyPigMultiboxCheckbox42", "Heal - Normal" },
		[4] = { "LazyPigMultiboxCheckbox45", "Heal - Fast" },
...
local ButtonTables = {
	...
	["Group Management"] = {
		...
		[1] = { "LazyMultiboxMakeLeader", "Make Me Leader", LazyPigMultibox_MakeMeLeader },
		[2] = { "LazyMultiboxConvert", "Convert to Raid", ConvertToRaid },
		[3] = { "LazyPigMultiboxDisband", "Disband Group", function() LazyPigMultibox_Annouce("lpm_hide_menu", "slave_only") LazyPigMultibox_DisbangGroup() end },
		[4] = { "LazyPigMultiboxAOE", "AoE Invite", function() LazyPigMultibox_AOEInvite(true) end },
		[5] = { "LazyPigMultiboxFriends", "Invite Friends", LazyPigMultibox_InviteFriends },
		[6] = { "LazyPigMultiboxGuildMates", "Invite Guildmates", LazyPigMultibox_InviteGuildMates },
	},
}
...
function LazyPigMultibox_CreateOptionsFrame()
	...
	frame.cbgroup_masterfollow = CheckBoxGroup(frame, 190, -100, str, CheckBoxTables[str])

	local str = "Follow Master Actions"
	frame.cbgroup_masteraction = CheckBoxGroup(frame, 190, -205, str, CheckBoxTables[str])

	local str = "Master Event-Handler"
	frame.cbgroup_eventhandler = CheckBoxGroup(frame, 190, -310, str, CheckBoxTables[str])

	local str = "Assist Master"
	frame.cbgroup_masterassist = CheckBoxGroup(frame, 380, -100, str, CheckBoxTables[str])

	local str = "Redirect Message --> Master"
	frame.cbgroup_messages = CheckBoxGroup(frame, 380, -190, str, CheckBoxTables[str])

	local str = "Use Predefined Class' Script"
	frame.cbgroup_scripts = CheckBoxGroup(frame, 380, -255, str, CheckBoxTables[str])

	-- Button Groups
	local str = "Group Loot Management"
	--ButtonGroup(frame, 20, -100, str, ButtonTables[str], false)
	frame.btngroup_loot = ButtonGroup(frame, 95, -100, str, ButtonTables[str], true)

	local str = "Group Action Manager"
	--ButtonGroup(frame, 20, -170, str, ButtonTables[str], false)
	frame.btngroup_actions = ButtonGroup(frame, 95, -170, str, ButtonTables[str], true)

	local str = "Group Management"
	--ButtonGroup(frame, 20, -240, str, ButtonTables[str], false)
	frame.btngroup_management = ButtonGroup(frame, 95, -240, str, ButtonTables[str], true)
