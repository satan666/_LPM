...
LPMULTIBOX = {
	...
	STATUS = true,
	FM_ALWAYS = true,
	FM_NOENEMYINDOORS = false,
	FM_NOENEMYOUTDOORS = false,
	FM_COMABTENDS = false,
	FM_SPELLFAIL = false,
	AM_FRIEND = false,
	AM_KEEPTARGET = false,
	AM_ENEMY = true,
	AM_ACTIVEENEMY = false,
	AM_ACTIVENPCENEMY = false,
	SM_SLAVELOST = true,
	SM_SPELLFAIL = true,
	SM_REDIRECT = true,
	FA_RELEASE = true,
...
-- Printing and DebugPrinting functions
local lpm_debug = false

function LPM_DEBUG(text)
	local header = "|cffCC2222[LPM] "
	if lpm_debug then
...
-- Let's create the main-Frame. This is replacing LazyPigMultibox.xml
LazyPigMultibox = CreateFrame("Frame", "LazyPigMultibox")
LazyPigMultiboxAnnouceText = LazyPigMultibox:CreateFontString("LazyPigMultiboxAnnouceText", "ARTWORK")
...
function LazyPigMultibox_OnEvent()
...
LazyPigMultibox:SetScript("OnEvent", LazyPigMultibox_OnEvent)
LazyPigMultibox:SetScript("OnUpdate", LazyPigMultibox_OnUpdate)
...
function LazyPigMultibox_MenuSet()
	...
	function Buttons2(val)
		local Buttons2Matrix = {
			...
			"LPM_CheckButton52",
			--"LazyMultiboxMacro",
			"LPM_OptionsFrame_SyncButton",
			"LPM_OptionsFrame_SyncButtonExt", --commodity "frame" to gray out the external fontstring
			--"LazyMultiboxQHCFG",
			--"LazyMultiboxLPCFG"
		}
	...
	if LPMULTIBOX.STATUS then
		...
		getglobal("LPM_OptionsFrame_EnableCheckButton"):SetChecked(true)
		getglobal("LPM_OptionsFrame_EnableCheckButtonText"):SetText("Multibox Enabled")
		if leader then
			Buttons1(true);
			getglobal("LPM_OptionsFrame_SyncButtonExtText"):SetText("Upload Settings");
			...
			Buttons1(nil);
			getglobal("LPM_OptionsFrame_SyncButtonExtText"):SetText("Request Settings");
		...
		getglobal("LPM_OptionsFrame_EnableCheckButton"):SetChecked(false)
		getglobal("LPM_OptionsFrame_EnableCheckButtonText"):SetText("Multibox Disabled")
	end
	...
	if class == "Rogue" or class == "Warrior" or class == "Hunter" or class == "Mage" or class == "Warlock" then
		...
		getglobal("LPM_OptionsFrame_QHCFGButton"):Disable();
		getglobal("LPM_OptionsFrame_QHCFGButtonExtText"):SetTextColor(.5, .5, .5)
	end
	if class == "Druid" then
		LPMULTIBOX.SCRIPT_REZ = nil
		getglobal("LPM_CheckButton43"):Disable();
